...
public class Acs: Codable, CustomStringConvertible, Equatable {
    ...
    public var missing: AcsHelper? {
        return AcsHelper.diff(a1: want, a2: given)
    }

    public var excessive: AcsHelper? {
        return AcsHelper.diff(a1: given, a2: want)
    }
    ...
    init(given: String?, want: String?, mode: String?) {
        assign(given: given, want: want, mode: mode)
    }
    ...
    init(from dict: [String: String]?) {
        if let d = dict {
            assign(given: d["given"], want: d["want"], mode: d["mode"])
        }
    }
    ...
    public func serialize() -> String {
        return [mode?.description ?? "",
                want?.description ?? "",
                given?.description ?? ""].joined(separator: ",")
    }

    public static func deserialize(from data: String?) -> Acs? {
