...
public class MsgClientResend: Codable {
    public let id: String?
    public let scheme: String?
...
public class MsgClientFriend: Codable {
    public let id: String?
    public let scheme: String?
    public let login: Bool?
    public let find: String?
    public let user: String?
    public let insert: [[String: JSONValue]]?
...
public class MsgClientGrpChat: Codable {
    public let id: String?
    public let scheme: String?
    public let topic: String?
    public let memberToDel: [String]?
         ...
         queryId: String? = nil)
    {
        ...
        self.scheme = scheme
        self.topic = topic
        self.memberToDel = memberToDel
        self.queryId = queryId
    }
}

public class MsgClientGetKey: Codable {
    public let id: String?
    public let scheme: String?
    public let user: String?
...
public class MsgClientKycKey: Codable {
    public let id: String?
    public let scheme: String?
    public let user: String?
...
public class MsgClientAppVersion: Codable {
    public let id: String?
    public let scheme: String?
...
public class MsgClientLog: Codable {
    public let id: String?
...
public class MsgClientPaySever: Codable {
    public let id: String?
    public let scheme: String?
...
public class MsgGetMeta: CustomStringConvertible, Codable {
    ...
    init() {
        what = "\(MsgGetMeta.kDesc) \(MsgGetMeta.kData) \(MsgGetMeta.kDel) \(MsgGetMeta.kTags)"
    }
    ...
    private func buildWhat() {
        var parts: [String] = []
        if (set & MsgGetMeta.kDescSet) != 0 || desc != nil {
            parts.append(MsgGetMeta.kDesc)
        }
        if (set & MsgGetMeta.kSubSet) != 0 || sub != nil {
            parts.append(MsgGetMeta.kSub)
        }
        if (set & MsgGetMeta.kDataSet) != 0 || data != nil {
            parts.append(MsgGetMeta.kData)
        }
        if (set & MsgGetMeta.kDelSet) != 0 || del != nil {
            parts.append(MsgGetMeta.kDel)
        }
        if (set & MsgGetMeta.kTagsSet) != 0 {
            parts.append(MsgGetMeta.kTags)
        }
        if (set & MsgGetMeta.kCredSet) != 0 {
            parts.append(MsgGetMeta.kCred)
        }
        what = parts.joined(separator: " ")
    }
...
public class MetaSetDesc<P: Codable, R: Codable>: Codable {
    ...
    public init(auth: String, anon: String) {
        defacs = Defacs(auth: auth, anon: anon)
    }
}
...
public class MsgClientDel: Codable {
    ...
    init(id: String?, topic: String?, what: String?, ranges: [MsgRange]?, user: String?, cred: Credential?, hard: Bool?) {
        ...
        delseq = what == MsgClientDel.kStrMsg ? ranges : nil
    ...
    convenience init(id: String?, cred: Credential) {
        self.init(id: id, topic: BudChat.kTopicMe, what: MsgClientDel.kStrCred,
...
public class ClientMessage<Pu: Codable, Pr: Codable>: Codable {
    ...
    public var credResend: MsgClientResend?
    public var NewFriend: MsgClientFriend?
    public var getKey: MsgClientGetKey?
    public var grpChat: MsgClientGrpChat?
    public var appVersion: MsgClientAppVersion?
    public var log: MsgClientLog?
    public var paySever: MsgClientPaySever?
    public var kycKey: MsgClientKycKey?
    ...
    init(resend: MsgClientResend) {
        credResend = resend
    }

    init(NewFriend: MsgClientFriend) {
        self.NewFriend = NewFriend
    }

    init(getKey: MsgClientGetKey) {
        self.getKey = getKey
    }

    init(grpChat: MsgClientGrpChat) {
        self.grpChat = grpChat
    }

    init(appVersion: MsgClientAppVersion) {
        self.appVersion = appVersion
    }

    init(log: MsgClientLog) {
        self.log = log
    }

    init(paySever: MsgClientPaySever) {
        self.paySever = paySever
    }

    init(kycKey: MsgClientKycKey) {
        self.kycKey = kycKey
    }
}
