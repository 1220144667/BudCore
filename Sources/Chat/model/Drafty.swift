...
open class Drafty: Codable, CustomStringConvertible, Equatable {
    ...
    public static let tfConstant = "TF"
    public static let bcConstant = "BC"
    ...
    public var entReferences: [String]? {
        ...
        for anEnt in ent {
            if let ref = anEnt.data?["ref"] {
                if case let .string(str) = ref {
                    result.append(str)
                }
            }
            if let preref = anEnt.data?["preref"] {
                if case let .string(str) = preref {
                    result.append(str)
                }
            }
        }
    ...
    ///
    /// - Parameters:
    ///     - at: location to insert Contact at
    /// - Returns: 'self' Drafty object.
    public func insertContact(at: Int,
                              topicId: String,
                              name: String,
                              headUrl: String,
                              phoneNum: String,
                              isRegister: Bool,
                              relation: String) throws -> Drafty
    {
//        guard bits != nil || refurl != nil else {
//            throw DraftyError.illegalArgument("Either image bits or reference URL must not be null.")
//        }
        ...
        var data: [String: JSONValue] = [:]
        if !topicId.isEmpty {
            data["topicId"] = JSONValue.string(topicId)
        }
        if !name.isEmpty {
            data["name"] = JSONValue.string(name)
        }
        if !headUrl.isEmpty {
            data["headUrl"] = JSONValue.string(headUrl)
        }
        if !phoneNum.isEmpty {
            data["phoneNum"] = JSONValue.string(phoneNum)
        }
        data["isRegister"] = JSONValue.bool(isRegister)
        if !relation.isEmpty {
            data["relation"] = JSONValue.string(relation)
        }
        ent!.append(Entity(tp: Drafty.bcConstant, data: data))
    ...
    ///
    /// - Returns: 'self' Drafty object.
    func insertButton(at: Int, len: Int, name: String?, actionType: String, actionValue: String?, refUrl: URL?) throws -> Drafty {
    ...
    fileprivate class SpanTreeProcessor {
        ...
        private class func spansToTree(tree parent: Span, line: String, start startAt: Int, end: Int, spans: [Span]) -> Span {
            var start = startAt
            ...
            while i < spans.count {
                ...
                parent.append(spansToTree(tree: span, line: line, start: start, end: span.end, spans: subspans))
        ...
        public class func toTree(contentOf content: Drafty) -> Span? {
            let txt = content.txt
            var fmt = content.fmt
            let ent = content.ent
    ...
    private class ShorteningTransformer: DraftyTransformer {
        ...
        public init(length: Int, tail: String?) {
            self.tail = tail
            tailLen = tail?.count ?? 0
            limit = length - tailLen
        }
    ...
    public func preview(previewLen: Int) -> Drafty {
        ...
        tree = SpanTreeProcessor.treeTopDown(tree: tree, using: LightCopyTransformer(
            allowedFields: ["state", "incoming", "preview", "preref", "val", "ref"], forTypes: ["IM", "VC", "VD"]
        )) ?? tree
    ...
    public func toMarkdown(withPlainLinks plainLinks: Bool) -> String {
        ...
        class Formatter: DraftyFormatter {
            ...
            func apply(type: String?, data: [String: JSONValue]?, key _: Int?, content: [FormattedString], stack _: [String]?) -> FormattedString {
                var res = ""
                for ws in content {
    ...
    public func updateVideoEnt(withParams params: [String: JSONValue]?, isIncoming: Bool) {
        guard let fmt = fmt, !fmt.isEmpty, let params = params, fmt.first!.tp != "VC" else {
            return
        }
        let st = fmt.first!
        if st.tp != nil {
        ...
        guard let ent = ent, !ent.isEmpty, ent.first!.tp == "VC" else { return }
        let e = ent.first!
        if e.data == nil {
            e.data = [:]
        }
    ...
    public class Span {
        ...
        func appendTo(document doc: inout Drafty, withKeymap keymap: inout [Int: Int]) {
            ...
            if let tp = type {
                ...
                if let attr = data, !attr.isEmpty {
                    // Got entity.
                    if doc.ent == nil { doc.ent = [] }
                    var newKey: Int
                    if let oldKey = keymap[key] {
                        newKey = oldKey
                    } else {
...
public class Entity: Codable, CustomStringConvertible, Equatable {
    ...
    public func copyLight() -> Entity {
        ...
        return Entity(tp: tp, data: dataCopy)
    }
}
