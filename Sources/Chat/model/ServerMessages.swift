...
public class MsgServerCtrl: Codable {
    ...
    public func getStringArray(for key: String) -> [String]? {
        if case let .array(v)? = params?[key] {
    ...
    // Testing only.
    init(id: String?, topic: String?, code: Int, text: String, ts: Date, params: [String: JSONValue]?) {
        self.id = id
        self.topic = topic
        self.code = code
        self.text = text
...
public class MsgServerMeta: Codable {
    ...
    public required init(from decoder: Decoder) throws {
        ...
        cred = try? container.decode([Credential].self, forKey: .cred)
        if topic == BudChat.kTopicMe {
            desc = try? container.decode(DefaultDescription.self, forKey: .desc)
            sub = try? container.decode([DefaultSubscription].self, forKey: .sub)
        } else if topic == BudChat.kTopicFnd {
            desc = try? container.decode(FndDescription.self, forKey: .desc)
            sub = try? container.decode([FndSubscription].self, forKey: .sub)
        } else {
            desc = try? container.decode(DefaultDescription.self, forKey: .desc)
            sub = try? container.decode([DefaultSubscription].self, forKey: .sub)
        }
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        if topic == BudChat.kTopicMe {
            ...
            if let sub = sub as? [DefaultSubscription] { try container.encode(sub, forKey: .sub) }
        } else if topic == BudChat.kTopicFnd {
            if let desc = desc as? FndDescription { try container.encode(desc, forKey: .desc) }
            if let sub = sub as? [FndSubscription] { try container.encode(sub, forKey: .sub) }
        } else {
            if let desc = desc as? DefaultDescription { try container.encode(desc, forKey: .desc) }
            if let sub = sub as? [DefaultSubscription] { try container.encode(sub, forKey: .sub) }
        }
    ...
    // Testing only.
    init(id: String?, topic: String?, ts: Date, desc: DescriptionProto?,
...
open class MsgServerData: Codable {
    ...
    public var replacesSeq: Int? {
        // Returns seq id (if any) this message is intended to replace.
        guard let replaceStr = head?["replace"]?.asString() else {
            return nil
        }
    ...
    public var webrtcCallState: WebRTC? {
        guard let str = head?["webrtc"]?.asString() else { return nil }
    ...
    public var isAudioOnlyCall: Bool {
        return head?["aonly"]?.asBool() ?? false
    }

    // Testing only.
    init(id: String?, topic: String?, from: String?, ts: Date?,
...
public class MsgServerPay: Codable {
    public var id: String?
    public let params: [String: JSONValue]?
    public var code: Int
...
public class ServerMessage: Codable {
    ...
    public static let kStatusTokenExpired = 416 // 3.1
    ...
    public static let kStatusLogout = 604
    // 被拉黑会返回、拉黑专用、如果不是就去屌后端
    public static let kStatusBlock = 608
