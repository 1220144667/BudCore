...
open class MeTopic<DP: Codable & Mergeable>: Topic<DP, PrivateType, DP, PrivateType>, MeTopicProto {
    open class Listener: Topic<DP, PrivateType, DP, PrivateType>.Listener {
        ...
        open func onCredUpdated(cred _: [Credential]?) {}
    }
    ...
    public init(tinode: BudChat?, l: MeTopic<DP>.Listener? = nil) {
        super.init(bud: tinode, name: BudChat.kTopicMe, l: l)
    }

    public init(tinode: BudChat?, desc: Description<DP, PrivateType>) {
        super.init(bud: tinode, name: BudChat.kTopicMe, desc: desc)
    }

    public func serializeCreds() -> String? {
        guard let c = creds else { return nil }
        return BudChat.serializeObject(c)
    }

    public func deserializeCreds(from data: String?) -> Bool {
        if let c: [Credential] = BudChat.deserializeObject(from: data) {
            creds = c
            return true
        }
        return false
    ...
    override func update(acsMap: [String: String]?, sub: MetaSetSub) {
        var newAcs: Acs
    ...
    override func update(ctrl: MsgServerCtrl, meta: MsgSetMeta<DP, PrivateType>) {
    ...
    public func setMsgReadRecv(from topicName: String?, what: String?, seq: Int?) {
        guard let tn = topicName, let topic = bud?.getTopic(topicName: tn), let seq = seq else { return }
    ...
    override public func routeInfo(info: MsgServerInfo) {
        guard let what = info.what, what != BudChat.kNoteKp, let src = info.src else { return }
        if let t = bud!.getTopic(topicName: src) as? DefaultTopic {
    ...
    override public func routePres(pres: MsgServerPres) {
        ...
        if what == .kUpd && BudChat.kTopicMe == pres.src {
        ...
        if let topic = bud!.getTopic(topicName: pres.src ?? "") {
            switch what {
            ...
            case .kMsg: // new message received
                ...
                if pres.act == nil || bud!.isMe(uid: pres.act!) {
            ...
            case .kUpd: // pub/priv updated
                ...
                let src = pres.src ?? ""
                if src.contains("usr") {
                    NotificationCenter.default.post(name: NSNotification.Name("autoImportContacts"), object: nil)
                }
                if src.contains("grp") {
                    NotificationCenter.default.post(name: NSNotification.Name("updateGroups"), object: nil)
                }
            ...
            case .kGone:
                if topic.deleted {
                    ...
                    bud!.stopTrackingTopic(topicName: pres.src!)
                } else {
                    // Mark as deleted.
                ...
                let src = pres.src ?? ""
                if src.contains("grp") {
                    NotificationCenter.default.post(name: NSNotification.Name("deleteGroup"), object: src)
                }
            case .kDel: // messages deleted
                // Explicitly ignored: 'me' topic has no messages.
                break
            default:
                BudChat.log.error("ME.pres message - unknown what: %@", String(describing: pres.what))
            }
        } else {
            // nil (me) or a previously unknown topic
            switch what {
            case .kAcs:
                if pres.src != nil && pres.src != BudChat.kTopicMe {
                    let acs = Acs()
                    acs.update(from: pres.dacs)
                    if acs.isModeDefined {
                        getMeta(query: metaGetBuilder().withSub(user: pres.src).build())
                    } else {
                        BudChat.log.error("ME.acs - unexpected access mode: %@", String(describing: pres.dacs))
                    }
                }
                if let src = pres.src, src.contains("usr") {
                    NotificationCenter.default.post(name: NSNotification.Name("autoImportContacts"), object: nil)
                }
                if let src = pres.src, src.contains("grp") {
                    NotificationCenter.default.post(name: NSNotification.Name("updateGroups"), object: nil)
                }
            case .kTags:
            ...
            default:
                BudChat.log.error("ME.pres - topic not found: what = %@, src = %@",
    ...
    fileprivate func assignRecv(to topic: TopicProto, recv seq: Int?) {
        if (topic.recv ?? -1) < (seq ?? -1) {
            topic.recv = seq
            store?.setRecv(topic: topic, recv: seq!)
        }
    }
    ...
    fileprivate func assignRead(to topic: TopicProto, read seq: Int?) {
        if (topic.read ?? -1) < (seq ?? -1) {
            topic.read = seq
            store?.setRead(topic: topic, read: topic.read!)
    ...
    override func routeMetaSub(meta: MsgServerMeta) {
        if let metaSubs = meta.sub as? [Subscription<DP, PrivateType>] {
            for sub in metaSubs {
                if let topicValue = sub.topic, let topic = bud!.getTopic(topicName: topicValue) {
                    if sub.deleted != nil {
                        if topic.deleted {
                            topic.expunge(hard: true)
                            bud!.stopTrackingTopic(topicName: sub.topic!)
                        } else {
                            topic.expunge(hard: false)
                        }
                    } else {
                        if let t = topic as? DefaultTopic {
                            ...
                            BudChat.log.fault("ME.routeMetaSub - failed to update topic %@", String(describing: topic))
                            assertionFailure()
                        }
                    }
                    ...
                    if sub.topic != nil {
                        ...
                        let topic = bud!.newTopic(sub: sub)
                        topic.persist()
                    }
                }
    ...
    func routeMetaCred(cred: Credential) {
        processOneCred(cred)
