...
public extension Message {
    ...
    // not a protocol.
    func equals(_ other: Message) -> Bool {
        return
            msgId == other.msgId &&
            seqId == other.seqId &&
            ts == other.ts &&
...
public protocol Storage: AnyObject {
    ...
    // Fetch all topics.
    func topicGetAll(from tinode: BudChat?) -> [TopicProto]?
    // query all topics.
    func queryAll(from tinode: BudChat?, keyworld: String) -> [TopicProto]
    // Retrieve a topic by name.
    func topicGet(from tinode: BudChat?, withName name: String?) -> TopicProto?
    // Add new topic.
    @discardableResult
