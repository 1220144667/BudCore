...
//  ios
//
//  Copyright © 2019-2021 BudChat LLC. All rights reserved.
//
...
open class Topic<DP: Codable & Mergeable, DR: Codable & Mergeable, SP: Codable, SR: Codable>: TopicProto {
    ...
    open class Listener {
        public init() {}
        open func onSubscribe(code _: Int, text _: String) {}
        open func onLeave(unsub _: Bool?, code _: Int?, text _: String?) {}

        // Process {data} message.
        open func onData(data _: MsgServerData?) {}
        // All requested data messages received.
        open func onAllMessagesReceived(count _: Int) {}

        // {info} message received.
        open func onInfo(info _: MsgServerInfo) {}
        // {meta} message received.
        open func onMeta(meta _: MsgServerMeta) {}
        // {meta what="sub"} message received, and this is one of the subs.
        open func onMetaSub(sub _: Subscription<SP, SR>) {}
        // {meta what="desc"} message received.
        open func onMetaDesc(desc _: Description<DP, DR>) {}
        // {meta what="tags"} message received.
        open func onMetaTags(tags _: [String]) {}
        // {meta what="sub"} message received and all subs were processed.
        open func onSubsUpdated() {}
        // {pres} received.
        open func onPres(pres _: MsgServerPres) {}
        // {pres what="on|off"} is received.
        open func onOnline(online _: Bool) {}
        // Topic descriptor as contact is updated.
        open func onContUpdate(sub _: Subscription<SP, SR>) {}
    }
    ...
    var subs: [String: Subscription<SP, SR>]?
    public var tags: [String]?
    private var lastKeyPress: Date = .init(timeIntervalSince1970: 0)
    ...
    public var topicType: TopicType {
        return BudChat.topicTypeByName(name: name)
    }
    ...
    public var isGivenManager: Bool {
        return description.acs?.given?.isManager ?? false
    }
    ...
    public var maxRecvReadValues: (Int, Int) {
        ...
        guard let subs = subs, let me = bud?.myUid else { return (0, 0) }
    ...
    public weak var store: Storage?
    public var payload: Payload?
    public var isPersisted: Bool { return payload != nil }
    ...
    func msgReadRecvCount(seq: Int?, read: Bool) -> Int {
        if subs == nil {
            loadSubs()
        }

        guard let subs = subs, let me = bud?.myUid, let seq = seq, seq > 0 else { return 0 }
    ...
    init(bud: BudChat?, name: String, desc: Description<DP, DR>? = nil, l: Listener? = nil) {
        self.bud = bud
        self.name = name
        ...
        if bud != nil {
            bud!.startTrackingTopic(topic: self)
        }
    }

    convenience init(tinode: BudChat?, sub: Subscription<SP, SR>) {
        self.init(bud: tinode, name: sub.topic!)
    ...
    public static func isNewByName(name: String) -> Bool {
        return name.starts(with: BudChat.kTopicNew) || name.starts(with: BudChat.kChannelNew)
    }
    ...
    public func serializeTrusted() -> String? {
        ...
        return BudChat.serializeObject(t)
    }

    public func deserializePub(from data: String?) -> Bool {
        if let p: DP = BudChat.deserializeObject(from: data) {
            description.pub = p
            return true
        }
        return false
    ...
    public func deserializePriv(from data: String?) -> Bool {
        if let p: DR = BudChat.deserializeObject(from: data) {
            description.priv = p
            return true
        }
        return false
    ...
    public func deserializeTrusted(from data: String?) -> Bool {
        if let t: TrustedType = BudChat.deserializeObject(from: data) {
    ...
    public func persist() {
        if !isPersisted {
            ...
            if isP2PType {
                bud?.updateUser(uid: name, desc: description)
            }
        }
    }
    ...
    func subscribe() -> PromisedReply<ServerMessage> {
        ...
        if isNew {
            setMsg = MsgSetMeta<DP, DR>(
                desc: MetaSetDesc(pub: pub, priv: priv),
                sub: nil,
                tags: tags, cred: nil
            )
        } else {
    ...
    public func subscribe(set: MsgSetMeta<DP, DR>?, get: MsgGetMeta?) -> PromisedReply<ServerMessage> {
        ...
        guard tnd.isConnected else {
            return PromisedReply(error: ChatError.notConnected("Cannot subscribe to topic. No server connection."))
        }
        guard tnd.isConnectionAuthenticated else {
            return PromisedReply(error: ChatError.invalidState("Connection is not authenticated."))
        }
        return tnd.subscribe(to: name, set: set, get: get).then(
                    ...
                    if let ctrl = msg?.ctrl {
                        if !(ctrl.params?.isEmpty ?? true) {
                            ...
                            if self?.isNew ?? false {
                                ...
                                _ = self?.bud?.changeTopicName(topic: self!, oldName: name)
                            }
                            // update store
                            self?.store?.topicUpdate(topic: self!)
                            if let t = self, t.isP2PType {
                                t.bud?.updateUser(uid: t.name, desc: t.description)
                            }
                        }
            ...
            onFailure: { [weak self] err in
                if let e = err as? ChatError, self?.isNew ?? false,
                   case let ChatError.serverResponseError(code, _, _) = e
                {
                    if ServerMessage.kStatusBadRequest <= code, code < ServerMessage.kStatusInternalServerError {
                        self?.bud?.stopTrackingTopic(topicName: name)
    ...
    @discardableResult func loadSubs() -> Int {
    ...
    private func routeMetaDesc(meta: MsgServerMeta) {
        ...
        if case .p2p = topicType {
            bud?.updateUser(uid: name, desc: meta.desc as! DefaultDescription)
        }
        // update listener
    ...
    func update(sub: Subscription<SP, SR>) {
    ...
    func update(desc: Description<DP, DR>) {
    ...
    func update(desc: MetaSetDesc<DP, DR>) {
        if description.merge(desc: desc) {
            store?.topicUpdate(topic: self)
            if isP2PType {
                bud?.updateUser(uid: name, desc: description)
    ...
    func update(ctrl: MsgServerCtrl, meta: MsgSetMeta<DP, DR>) {
        if let desc = meta.desc {
            update(desc: desc)
            listener?.onMetaDesc(desc: description)
        }
        if let sub = meta.sub {
            let acsMap = ctrl.getStringDict(for: "acs")
            update(acsMap: acsMap, sub: sub)
            if sub.user == nil {
                listener?.onMetaDesc(desc: description)
            }
            listener?.onSubsUpdated()
    ...
    func update(acsMap: [String: String]?, sub: MetaSetSub) {
        var user = sub.user
        var acs: Acs
        ...
        if user == nil || (bud?.isMe(uid: user) ?? false) {
            user = bud?.myUid
            var changed = false
            if description.acs == nil {
                ...
                description.acs!.merge(from: acs)
            }
            if changed {
                store?.topicUpdate(topic: self)
                if isP2PType {
        ...
        if let sub2 = getSubscription(for: user) {
            ...
            sub2.user = user
            sub2.acs = acs
            addSubToCache(sub: sub2)
            _ = store?.subNew(topic: self, sub: sub2)
        }
    }

    func addSubToCache(sub: Subscription<SP, SR>) {
    ...
    private func processSub(newsub: Subscription<SP, SR>) {
        ...
        if newsub.deleted != nil {
            ...
            guard let user = newsub.user else {
                BudChat.log.error("Subscription missing user field (topic %@), uniqueId: %@", name, newsub.uniqueId ?? "NONE")
                return
            }
            ...
            bud!.updateUser(sub: sub!)
            // If this is a change to user's own permissions, update topic too.
            if bud!.isMe(uid: newsub.user) && newsub.acs != nil {
    ...
    public func delete(hard: Bool) -> PromisedReply<ServerMessage> {
        if deleted {
            topicLeft(unsub: true, code: 200, reason: "OK")
            bud!.stopTrackingTopic(topicName: name)
            expunge(hard: true)
            return PromisedReply(value: nil)
        }

        // Delete works even if the topic is not attached.
        return bud!.delTopic(topicName: name, hard: hard).then(
            onSuccess: { msg in
                self.topicLeft(unsub: true, code: msg?.ctrl?.code, reason: msg?.ctrl?.text)
                self.bud!.stopTrackingTopic(topicName: self.name)
                self.expunge(hard: true)
                return nil
            }
        )
    ...
    private func note(what: NoteType, fromMe: Bool = true, explicitSeq: Int? = nil) -> Int {
        ...
        case .kRecv:
            ...
            if description.getRecv < seq {
                if !fromMe {
                    bud!.noteRecv(topic: name, seq: seq)
                }
                result = seq
        ...
        case .kRead:
            ...
            if explicitSeq != nil || description.getRead < seq {
                if !fromMe {
                    bud!.noteRead(topic: name, seq: explicitSeq ?? seq)
                }
    ...
    public func videoCall(event: String, seq: Int, payload: JSONValue? = nil) {
        bud?.videoCall(topic: name, seq: seq, event: event, payload: payload)
    }
    ...
    public func routeData(data: MsgServerData) {
        ...
        if let s = store {
            if let msg = s.msgReceived(topic: self, sub: getSubscription(for: data.from), msg: data) {
                latestMessage = msg
                noteRecv(fromMe: bud!.isMe(uid: data.from))
            }
        } else {
            noteRecv(fromMe: bud!.isMe(uid: data.from))
        }
        ...
        if let me = bud?.getMeTopic() {
            me.setMsgReadRecv(from: name, what: "", seq: 0)
        }
    }

    @discardableResult
    public func getMeta(query: MsgGetMeta) -> PromisedReply<ServerMessage> {
        return bud!.getMeta(topic: name, query: query)
    }
    ...
    public func setMeta(meta: MsgSetMeta<DP, DR>) -> PromisedReply<ServerMessage> {
        ...
        return bud!.setMeta(for: name, meta: meta).thenApply { msg in
    ...
    public func updateMode(uid: String?, update: String) -> PromisedReply<ServerMessage> {
        var uid = uid
        let sub = getSubscription(for: uid ?? bud?.myUid)
        if uid == bud?.myUid {
            uid = nil
        }
    ...
    public func invite(user uid: String, in mode: String?) -> PromisedReply<ServerMessage> {
        ...
        if sub != nil {
            ...
            let user: User<SP>? = bud?.getUser(with: uid)
    ...
    public func eject(user uid: String, ban: Bool) -> PromisedReply<ServerMessage> {
        ...
        if isNew {
            ...
            listener?.onSubsUpdated()
            return PromisedReply(error: ChatError.notSynchronized)
        }
        return bud!.delSubscription(topicName: name, user: uid).thenApply { _ in
    ...
    public func setReadRecvByRemote(from uid: String?, what: String?, seq: Int?) {
        if let sub = getSubscription(for: uid) {
            switch what {
            case BudChat.kNoteRecv:
                sub.recv = seq
                store?.msgRecvByRemote(sub: sub, recv: seq)
            case BudChat.kNoteRead:
                sub.read = seq
    ...
    public func routeInfo(info: MsgServerInfo) {
        if info.what != BudChat.kNoteKp {
            setReadRecvByRemote(from: info.from, what: info.what, seq: info.seq)
            if let t = bud, t.isMe(uid: info.from), let me = t.getMeTopic() {
                me.setMsgReadRecv(from: name, what: info.what, seq: info.seq)
            }
        }
    ...
    public func routePres(pres: MsgServerPres) {
        ...
        case .kAcs:
            ...
            if sub == nil {
                ...
                if acs.isModeDefined {
                    ...
                    s.acs = acs
                    s.updated = Date()
                    if let user: User<SP> = bud!.getUser(with: userId) {
                        s.pub = user.pub
                    } else {
                    ...
                    BudChat.log.error("Invalid access mode update %@", pres.dacs?.description ?? "nil")
                }
            } else {
        ...
        default:
            BudChat.log.error("pres message - unknown what: %@", String(describing: pres.what))
        }
    ...
    public func leave(unsub: Bool? = false) -> PromisedReply<ServerMessage> {
        if attached {
            return bud!.leave(topic: name, unsub: unsub)
                .thenApply { [weak self] msg in
                    guard let s = self else {
                        throw ChatError.invalidState("Topic.self not available in result handler")
                    }
                    s.topicLeft(unsub: unsub, code: msg?.ctrl?.code, reason: msg?.ctrl?.text)
                    if unsub ?? false {
                        s.bud?.stopTrackingTopic(topicName: s.name)
                        s.expunge(hard: true)
                    }
                    return nil
                }
        ...
        if bud!.isConnected {
            return PromisedReply(error:
                ChatError.notSubscribed("Can't leave topic that I'm not subscribed to \(name)"))
        }
        return PromisedReply(
            error: ChatError.notConnected("Leaving topic when Tinode is not connected."))
    }
    ...
    private func publishInternal(content: Drafty, head: [String: JSONValue]?, msgId: Int64) -> PromisedReply<ServerMessage> {
        ...
        return bud!.publish(topic: name, head: headers, content: content, attachments: attachments).then(
            onSuccess: { [weak self] msg in
...
//        }
        if attached {
            ...
            return subscribe()
                .thenApply { [weak self] _ in
                    self?.publishInternal(content: content, head: head, msgId: id)
    ...
    private func sendPendingDeletes(hard: Bool) -> PromisedReply<ServerMessage>? {
        if let pendingDeletes = store?.getQueuedMessageDeletes(topic: self, hard: hard), !pendingDeletes.isEmpty {
            return bud!.delMessage(
                topicName: name, ranges: pendingDeletes, hard: hard
            )
            .thenApply { [weak self] msg in
    ...
    private func delMessages(from fromId: Int, to toId: Int, hard: Bool) -> PromisedReply<ServerMessage> {
        ...
        if attached {
            return bud!.delMessage(topicName: name, fromId: fromId, toId: toId, hard: hard).then(
                onSuccess: { [weak self] msg in
        ...
        if bud?.isConnected ?? false {
            return PromisedReply<ServerMessage>(error: ChatError.notSubscribed("Not subscribed to topic."))
        }
        return PromisedReply<ServerMessage>(error: ChatError.notConnected("Tinode not connected."))
    }
    ...
    private func delMessages(inRanges ranges: [MsgRange], hard: Bool) -> PromisedReply<ServerMessage> {
        ...
        if attached {
            return bud!.delMessage(topicName: name, ranges: ranges, hard: hard).then(
                onSuccess: { [weak self] msg in
                    if let s = self, let delId = msg?.ctrl?.getIntParam(for: "del"), delId > 0 {
                        if (s.clear ?? 0) < delId {
                            s.clear = delId
    ...
    public func delMessages(hard: Bool) -> PromisedReply<ServerMessage> {
        return delMessages(from: 0, to: (seq ?? 0) + 1, hard: hard)
    }
    ...
    public func syncOne(msgId: Int64) -> PromisedReply<ServerMessage> {
        ...
        if m.isDeleted {
            return bud!.delMessage(topicName: name, msgId: m.seqId, hard: m.isDeleted(hard: true))
        }
        if m.isReady, let content = m.content {
            store?.msgSyncing(topic: self, dbMessageId: msgId, sync: true)
            return publishInternal(content: content, head: m.head, msgId: msgId)
        }
        return PromisedReply<ServerMessage>(value: ServerMessage())
    }

    public func syncAll() -> PromisedReply<ServerMessage> {
        var result = PromisedReply<ServerMessage>(value: ServerMessage())
        // Soft deletes.
        if let r = sendPendingDeletes(hard: false) {
            result = r
        }
        // Hard deletes.
        if let r = sendPendingDeletes(hard: true) {
            result = r
        }

        // Pending messages.
        guard let pendingMsgs = store?.getQueuedMessages(topic: self) else {
            return result
        }
        for msg in pendingMsgs {
            ...
            if msg.head?["webrtc"]?.asString() != nil {
                ...
                store?.msgDiscard(topic: self, dbMessageId: msgId)
                continue
            }
            store?.msgSyncing(topic: self, dbMessageId: msgId, sync: true)
            result = publishInternal(content: msg.content!, head: msg.head, msgId: msgId)
        }
        return result
    }
    ...
    public func setSetAndFetch(newSeq: Int?) {
        ...
        setSeq(seq: newSeq)
        if !attached {
            subscribe(set: nil, get: metaGetBuilder().withLaterData(limit: limit).build()).thenApply { _ in
                #warning("代改")
//                self.leave()
                return nil
            }
