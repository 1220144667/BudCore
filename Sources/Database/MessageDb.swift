...
public class MessageDb {
    ...
    init(_ database: SQLite.Connection, baseDb: BaseDb) {
        db = database
        self.baseDb = baseDb
        table = Table(MessageDb.kTableName)
        id = Expression<Int64>("id")
        topicId = Expression<Int64?>("topic_id")
        userId = Expression<Int64?>("user_id")
        status = Expression<Int?>("status")
        sender = Expression<String?>("sender")
        ts = Expression<Date?>("ts")
        seq = Expression<Int?>("seq")
        high = Expression<Int?>("high")
        delId = Expression<Int?>("del_id")
        replSeq = Expression<Int?>("repl_seq")
        effectiveSeq = Expression<Int?>("effective_seq")
        effectiveTs = Expression<Date?>("effective_ts")

        head = Expression<String?>("head")
        content = Expression<String?>("content")
    }

    func destroyTable() {
        try! db.run(table.dropIndex(topicId, effectiveSeq.desc, ifExists: true))
        try! db.run(table.dropIndex(topicId, seq.desc, ifExists: true))
    ...
    func createTable() {
        ...
        try! db.run(table.createIndex(topicId, seq.desc, unique: true, ifNotExists: true))
        // Partial index over topicId + effectiveSeq.
        let partIdx = table.createIndex(topicId, effectiveSeq.desc, unique: true, ifNotExists: true)
        try! db.run([partIdx, "WHERE", (effectiveSeq != nil).asSQL()].joined(separator: " "))
    }
                           ...
                           withEffectiveTs effectiveTs: Date?) throws -> Int64
    {
        ...
        setters.append(sender <- msg.from)
        setters.append(ts <- msg.ts)
        setters.append(seq <- msg.seq)
        if let replaced = msg.replacesSeq {
            setters.append(replSeq <- replaced)
        }
        if let effSeq = effSeqId, effSeq > 0 {
            setters.append(effectiveSeq <- effSeq)
        }
        if let effTs = effectiveTs {
            setters.append(self.effectiveTs <- effTs)
        }
        if let h = msg.head {
            setters.append(head <- BudChat.serializeObject(h))
        }
        setters.append(content <- msg.content?.serialize())
        msg.msgId = try db.run(table.insert(setters))
        return msg.msgId
    }

    private func getActiveVersion(withEffectiveSeqId effSeq: Int, onTopic topicId: Int64) -> Row? {
        return try? db.pluck(table.filter(self.topicId == topicId && effectiveSeq == effSeq))
    }

    private func deactivateMessageVersion(withEffectiveSeq seqId: Int, onTopic topicId: Int64) throws {
        let record = table.filter(self.topicId == topicId && effectiveSeq == seqId)
        try db.run(record.update(effectiveSeq <- nil))
    }
    ...
    private func activateMessageVersion(withEffectiveSeq seqId: Int, withEffectiveTs ts: Date?, onTopic topicId: Int64, originalAuthor: String?) throws -> Bool {
        ...
        guard let rec = try db.pluck(table.select(id).filter(self.topicId == topicId && replSeq == seqId && sender == originalAuthor)
            .order(seq.desc))
        else {
            return false
        }
        let recId = rec[id]
        let record = table.filter(id == recId)
        return try db.run(record.update(effectiveSeq <- seqId, effectiveTs <- ts)) > 0
    }
    ...
    func updateStatusAndContent(msgId: Int64, status: BaseDb.Status?, content: Drafty?) -> Bool {
        let record = table.filter(id == msgId)
        var setters = [Setter]()
        if status != .undefined {
            setters.append(self.status <- status!.rawValue)
        }
    ...
    func delivered(msgId: Int64, ts: Date, seq: Int) -> Bool {
        let record = table.filter(id == msgId)
        do {
            return try db.run(record.update(
                status <- BaseDb.Status.synced.rawValue,
                self.ts <- ts, self.seq <- seq,
                effectiveSeq <- replSeq ?? seq,
                effectiveTs <- effectiveTs ?? ts
            )) > 0
        } catch SQLite.Result.error(message: let errMsg, code: let code, statement: _) {
    ...
    func deleteAll(forTopic topicId: Int64) -> Bool {
        ...
        let rows = table.filter(self.topicId == topicId)
        do {
            return try db.run(rows.delete()) > 0
        } catch SQLite.Result.error(message: let errMsg, code: let code, statement: _) {
    ...
    func deleteFailed(forTopic topicId: Int64) -> Bool {
        let rows = table.filter(self.topicId == topicId && status == BaseDb.Status.failed.rawValue)
        do {
            return try db.run(rows.delete()) > 0
        } catch SQLite.Result.error(message: let errMsg, code: let code, statement: _) {
    ...
    func delete(inTopic topicId: Int64, seqId: Int) -> Bool {
        let record = table.filter(self.topicId == topicId && seq == seqId)
        do {
            return try db.run(record.delete()) > 0
        } catch SQLite.Result.error(message: let errMsg, code: let code, statement: _) {
    ...
    private func readOne(r: Row, previewLen: Int = -1) -> StoredMessage {
        ...
        sm.msgId = r[id]
        sm.topicId = r[topicId]
        sm.userId = r[userId]
        sm.dbStatus = BaseDb.Status(rawValue: r[status] ?? 0) ?? .undefined
        sm.from = r[sender]
        sm.topic = baseDb.topicDb?.getTopicName(id: r[topicId])
        sm.ts = r[effectiveTs] ?? r[ts]
        sm.seq = r[effectiveSeq] ?? r[seq]
        sm.head = BudChat.deserializeObject(from: r[head])
        sm.content = Drafty.deserialize(from: r[content])
    ...
    func query(topicId: Int64?, from: Int, limit: Int, forward: Bool) -> [StoredMessage]? {
        ...
        var query = table
        query = forward ?
            query.filter(self.topicId == topicId && effectiveSeq > from && effectiveSeq != nil) :
            query.filter(self.topicId == topicId && effectiveSeq < from && effectiveSeq != nil)
        query = (forward ? query.order(effectiveSeq.asc) : query.order(effectiveSeq.desc)).limit(limit)

        do {
            var messages: [StoredMessage] = []
            for row in try db.prepare(query) {
                let sm = readOne(r: row)
                messages.append(sm)
            }
            return messages
        } catch SQLite.Result.error(message: let errMsg, code: let code, statement: _) {
    ...
    func query(keyword: String, from: Int, limit: Int?, offset: Int?) -> [StoredMessage]? {
        // 查询语句
        let likes = "plain:%\(keyword)" + "%"
        var query = table.select(table[*]).filter(content.like(likes) && effectiveSeq < from).order(ts.desc)
    ...
    func query(msgId: Int64?, previewLen: Int) -> StoredMessage? {
        ...
        let record = table.filter(id == msgId).select(table[*])
        if let row = try? db.pluck(record) {
            return readOne(r: row, previewLen: previewLen)
        }
        return nil
    }
    ...
    func queryDeleted(topicId: Int64?, hard: Bool) -> [MsgRange]? {
        ...
        let queryTable = table
            ...
            .select(delId, seq, high)
            .order(seq)
        do {
            var ranges = [MsgRange]()
            for row in try db.prepare(queryTable) {
                if let low = row[seq] {
                    ranges.append(MsgRange(low: low, hi: row[high]))
                }
            }
            return ranges
        } catch SQLite.Result.error(message: let errMsg, code: let code, statement: _) {
    ...
    func queryUnsent(topicId: Int64?) -> [Message]? {
        let queryTable = table
            .filter(self.topicId == topicId && status == BaseDb.Status.queued.rawValue)
            .select(table[*])
            .order(ts)
        do {
            var messages = [StoredMessage]()
    ...
    func fetchNextMissingRange(topicId: Int64) -> MsgRange? {
        let messages2 = table.alias("m2")
        let hiQuery = table
            .join(.leftOuter, messages2,
                  on: table[seq] == (messages2[high] ?? messages2[seq] + 1) && messages2[self.topicId] == topicId)
            .filter(table[self.topicId] == topicId &&
                table[seq] > 1 &&
                messages2[seq] == nil)

        guard let hi = try? db.scalar(hiQuery.select(table[seq].max)) else {
            // No gap is found.
            return nil
        }
        // Find the first present message with ID less than the 'hi'.
        let seqExpr = (high - 1) ?? seq
        let lowQuery = table
            .filter(self.topicId == topicId && seq < hi)
        let low: Int
    ...
    func queryLatest() -> [StoredMessage]? {
        ...
        .join(.leftOuter, messages2,
              on: table[topicId] == messages2[topicId] && table[effectiveSeq] < messages2[effectiveSeq])
        .join(.leftOuter, topics, on: table[topicId] == topics[topicDb.id])
        .filter(table[delId] == nil && messages2[delId] == nil && messages2[m2Id] == nil && table[effectiveSeq] != nil)
        do {
            var messages = [StoredMessage]()
    ...
    func getMessage(fromTopic topicId: Int64, byEffectiveSeq seqId: Int) -> StoredMessage? {
        let record = table.filter(self.topicId == topicId && effectiveSeq == seqId)
        if let row = try? db.pluck(record) {
            return readOne(r: row, previewLen: -1)
        }
        return nil
    }
    ...
    func getAllVersions(fromTopic topicId: Int64, forSeq seqId: Int, limit: Int?) -> [Int]? {
        var query = table.select(seq).filter(self.topicId == topicId && replSeq == seqId)
            .order(seq.desc)
        if let limit = limit {
