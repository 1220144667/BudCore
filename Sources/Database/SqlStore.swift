...
public class SqlStore: Storage {
    public var myUid: String? {
        ...
        set {
            dbh?.setUid(uid: newValue, credMethods: nil)
        }
    }

    public var deviceToken: String? {
        get { dbh?.accountDb?.getDeviceToken() }
        set { dbh?.accountDb?.saveDeviceToken(token: newValue) }
    }
    ...
    init(dbh: BaseDb) {
        self.dbh = dbh
        print("==multi-user database init store: \(dbh.getDbPath())")
    }
    ...
    var timeAdjustment: TimeInterval = .init(0)
    public var isReady: Bool { return dbh?.isReady ?? false }

    public func topicGetAll(from tinode: BudChat?) -> [TopicProto]? {
        guard let tdb = dbh?.topicDb, let rows = tdb.query() else {
            return nil
        }
    ...
    public func queryAll(from tinode: BudChat?, keyworld: String) -> [any TopicProto] {
        guard let tdb = dbh?.topicDb else { return [] }
        return tdb.query(for: tinode, keyword: keyworld).filter { $0.topicType != .grp }
    }

    public func topicGet(from tinode: BudChat?, withName name: String?) -> TopicProto? {
        guard let tdb = dbh?.topicDb else { return nil }
    ...
    public func topicAdd(topic: TopicProto) -> Int64 {
        ...
        return dbh?.topicDb?.insert(topic: topic) ?? 0
    }

    public func topicUpdate(topic: TopicProto) -> Bool {
        return dbh?.topicDb?.update(topic: topic) ?? false
    }
    ...
    public func topicDelete(topic: TopicProto, hard: Bool) -> Bool {
        ...
        do {
            if hard {
                ...
                dbh?.topicDb?.markDeleted(recordId: topicId)
            }
            return true
        } catch {
    ...
    public func setRead(topic: TopicProto, read: Int) -> Bool {
        ...
        return dbh?.topicDb?.updateRead(for: topicId, with: read) ?? false
    }

    public func setRecv(topic: TopicProto, recv: Int) -> Bool {
        guard let st = topic.payload as? StoredTopic,
              let topicId = st.id, topicId > 0 else { return false }
        return dbh?.topicDb?.updateRecv(for: topicId, with: recv) ?? false
    }
    ...
    public func subAdd(topic: TopicProto, sub: SubscriptionProto) -> Int64 {
        ...
        return dbh?.subscriberDb?.insert(for: topicId, with: .synced, using: sub) ?? 0
    }

    public func subUpdate(topic _: TopicProto, sub: SubscriptionProto) -> Bool {
        guard let ss = sub.payload as? StoredSubscription, let subId = ss.id, subId > 0 else {
            return false
        }
        return dbh?.subscriberDb?.update(using: sub) ?? false
    }
    ...
    public func subNew(topic: TopicProto, sub: SubscriptionProto) -> Int64 {
        ...
        return dbh?.subscriberDb?.insert(for: topicId, with: .queued, using: sub) ?? 0
    }

    public func subDelete(topic _: TopicProto, sub: SubscriptionProto) -> Bool {
        guard let ss = sub.payload as? StoredSubscription, let subId = ss.id, subId > 0 else {
            return false
        }
        return dbh?.subscriberDb?.delete(recordId: subId) ?? false
    }
    ...
    public func getSubscriptions(topic: TopicProto) -> [SubscriptionProto]? {
        ...
        return dbh?.subscriberDb?.readAll(topicId: topicId)
    }

    public func userGet(uid: String) -> UserProto? {
        return dbh?.userDb?.readOne(uid: uid)
    }

    public func userAdd(user: UserProto) -> Int64 {
        return dbh?.userDb?.insert(user: user) ?? 0
    }

    public func userUpdate(user: UserProto) -> Bool {
        return dbh?.userDb?.update(user: user) ?? false
    }
    ...
    public func msgReceived(topic: TopicProto, sub: SubscriptionProto?, msg: MsgServerData?) -> Message? {
        ...
        if let ss = sub?.payload as? StoredSubscription {
            ...
            topicId = st.id ?? -1
            userId = dbh?.userDb?.getId(for: msg.from) ?? -1
            if userId < 0 {
                // Create a placeholder user to satisfy foreign key constraint.
                if sub != nil {
                    userId = dbh?.userDb?.insert(sub: sub) ?? -1
                } else {
                    userId = dbh?.userDb?.insert(uid: msg.from, updated: msg.ts, serializedPub: nil) ?? -1
                }
            }
        }
    ...
    private func insertMessage(topic: TopicProto, data: Drafty, head: [String: JSONValue]?, initialStatus: BaseDb.Status) -> Message? {
        ...
        if myId < 0 {
            myId = dbh?.userDb?.getId(for: msg.from) ?? -1
        }
        msg.userId = myId
        let id = dbh?.messageDb?.insert(topic: topic, msg: msg) ?? -1
        return id > 0 ? msg : nil
    }

    public func msgSend(topic: TopicProto, data: Drafty, head: [String: JSONValue]?) -> Message? {
        return insertMessage(topic: topic, data: data, head: head, initialStatus: .undefined)
    }

    public func msgDraft(topic: TopicProto, data: Drafty, head: [String: JSONValue]?) -> Message? {
        return insertMessage(topic: topic, data: data, head: head, initialStatus: .draft)
    }

    public func msgDraftUpdate(topic _: TopicProto, dbMessageId: Int64, data: Drafty) -> Bool {
        return dbh?.messageDb?.updateStatusAndContent(
            msgId: dbMessageId,
            status: .undefined,
            content: data
        ) ?? false
    ...
    public func msgReady(topic _: TopicProto, dbMessageId: Int64, data: Drafty) -> Bool {
        return dbh?.messageDb?.updateStatusAndContent(
            msgId: dbMessageId,
            status: .queued,
            content: data
    ...
    public func msgSyncing(topic _: TopicProto, dbMessageId: Int64, sync: Bool) -> Bool {
        return dbh?.messageDb?.updateStatusAndContent(
            msgId: dbMessageId,
    ...
    public func msgFailed(topic _: TopicProto, dbMessageId: Int64) -> Bool {
        return dbh?.messageDb?.updateStatusAndContent(
            msgId: dbMessageId,
            status: .failed,
            content: nil
    ...
    public func msgPruneFailed(topic: TopicProto) -> Bool {
        guard let st = topic.payload as? StoredTopic, let topicId = st.id else { return false }
        return dbh?.messageDb?.deleteFailed(forTopic: topicId) ?? false
    }

    public func msgDiscard(topic _: TopicProto, dbMessageId: Int64) -> Bool {
        return dbh?.messageDb?.delete(msgId: dbMessageId) ?? false
    }

    public func msgDiscard(topic: TopicProto, seqId: Int) -> Bool {
        guard let st = topic.payload as? StoredTopic, let topicId = st.id else { return false }
        return dbh?.messageDb?.delete(inTopic: topicId, seqId: seqId) ?? false
    }
    ...
    public func msgMarkToDelete(topic: TopicProto, from idLo: Int, to idHi: Int, markAsHard: Bool) -> Bool {
        guard let st = topic.payload as? StoredTopic, let topicId = st.id else { return false }
        return dbh?.messageDb?.deleteOrMarkDeleted(topicId: topicId, delId: nil, from: idLo, to: idHi, hard: markAsHard) ?? false
    }
    ...
    public func msgMarkToDelete(topic: TopicProto, ranges: [MsgRange]?, markAsHard: Bool) -> Bool {
        ...
        return dbh?.messageDb?.deleteOrMarkDeleted(topicId: topicId, delId: nil, inRanges: ranges, hard: markAsHard) ?? false
    }
    ...
    public func getMessage(keyword: String, from: Int, limit: Int? = nil, offset: Int? = nil) -> [StoredMessage]? {
        return BaseDb.sharedInstance.messageDb?.query(keyword: keyword, from: from, limit: limit, offset: offset)
    }
}
